...
function initLaunchInterceptor() {
  ...
  patientLink.addEventListener('click', (e) => {
    if (!patientOnline) {
      e.preventDefault();
      openOfflineModal('PsychAI Patient View App', 'http://localhost:3000', 'http://localhost:3000');
    }
  });
  ...
  clinicianLink.addEventListener('click', (e) => {
    if (!clinicianOnline) {
      e.preventDefault();
      openOfflineModal('PsychAI Clinician View App', 'http://localhost:3001', 'http://localhost:3001');
    }
  });
...
function speakText(text, sender) {
  ...
  try {
    // Cancel active narrations immediately to prevent voice queue overlaps
    window.speechSynthesis.cancel();

    const utterance = new SpeechSynthesisUtterance(text);
    
    // Configure voice rate and pitch to be relaxing and calm
    utterance.rate = 0.88; 
    
    if (sender === 'ai') {
      utterance.pitch = 1.05; // Slightly higher, encouraging counseling tone
    } else {
      utterance.pitch = 0.92; // Slightly deeper patient tone
    }
